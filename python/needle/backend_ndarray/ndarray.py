...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        # BEGIN YOUR SOLUTION
        new_shape = [(sl.stop - sl.start + sl.step - 1) //
                     sl.step for sl in idxs]
        offset = 0
        for sl, st in zip(idxs, self._strides):
            offset += sl.start * st
        new_strides = tuple(
            [st * sl.step for st, sl in zip(self._strides, idxs)])
        return NDArray.make(shape=new_shape, strides=new_strides, device=self._device, handle=self._handle, offset=offset)
        # END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        new_offset = sum([(self.shape[axis] - 1) * self.strides[axis]
                         for axis in axes])
    ...
    def pad(self, axes):
        ...
        access = tuple([slice(l, l + n)
                       for (l, _), n in zip(axes, self.shape)])
        arr[access] = self
        return arr
        # END YOUR SOLUTION
